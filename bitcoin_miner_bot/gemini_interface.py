...
def _robust_generate_content(prompt: str):
    ...
    try:
        ...
        if response and response.candidates:
            # Gemini's response structure can vary, candidates is often a list.
            # We assume we want the text from the first part of the first candidate.
            if len(response.candidates) > 0 and response.candidates[0].content and len(response.candidates[0].content.parts) > 0:
                return response.candidates[0].content.parts[0].text
        logging.warning("Gemini response was empty or did not contain expected content parts.")
        return None
...
def semantically_filter_article(article_title: str, article_summary: str, article_text: str) -> bool:
    ...
    **Criteria for "Not Relevant":**
    ...
    if result:
        return result.strip() == "RELEVANT_BITCOIN_MINING"
    logging.error("Failed to get a response from Gemini for semantic filtering.")
    return False
...
def generate_tweet_content(article_title: str, article_content: str, article_url_title: str) -> str:
    ...
    truncated_content = article_content[:30000]
    ...
    **Format your output as follows:**
    ...
    if result:
        return result.strip()
    logging.error("Failed to get a response from Gemini for tweet content generation.")
    return "ERROR: Could not generate tweet content."
...
def generate_thematic_trends(brief_content_md: str) -> list[str]:
    """
